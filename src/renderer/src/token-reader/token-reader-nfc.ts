...
export class TokenReaderNFC extends TokenReader<TokenStateNFC> {
    ...
    nfc.on("reader", async (reader: NFCReader) => {
      if (readerNameRegexp.test(reader.name)) {
        ...
        reader.on("card", async (card) => {
          ...
          if (!card.uid)
            return this.update({
              ...
              error: {
                type: TokenErrorTypeNFC.UID_INVALID,
                details: "Could not read UID from card.",
              },
            });
          ...
          try {
            ...
            if (err instanceof Error) handleReaderError(reader, err);
            this.update({
              state: TokenStateType.ERROR,
              error: {
                type: TokenErrorTypeNFC.READ_INTERRUPTED,
                details: "Could not read NDEF data from card.",
              },
            });
          }
        });
        ...
        reader.on("card.off", (card) => {
          ...
          if (this.token.state === TokenStateType.PRESENT && this.token.token.id === card.uid) {
            ...
            // tracking.
            this.token = { state: TokenStateType.ABSENT };
            this.emit("update");
          }
        });
  ...
  protected decode(uid: string, bytes: Buffer<ArrayBufferLike>): void {
    ...
    if (cc.message.records.length < 1)
      return this.update({
        state: TokenStateType.ERROR,
        error: {
          type: TokenErrorTypeNFC.DATA_INVALID,
          details: "No NDEF Records found on card.",
        },
      });
    ...
    if (recordType !== "text")
      return this.update({
        ...
        error: {
          type: TokenErrorTypeNFC.DATA_INVALID,
          details: `NDEF record type should be 'text', but is '${recordType}'.`,
        },
      });
    ...
    try {
      return this.update({
        ...
        token: { id: uid, class: decoder.decode(data) },
      });
    } catch (decodeError) {
      return this.update({
        state: TokenStateType.ERROR,
        error: {
          type: TokenErrorTypeNFC.DATA_INVALID,
          details: `Could not decode NDEF text record: ${decodeError}`,
        },
      });
    }
  }

  protected update(tokenState: TokenStateNFC) {
