/**
 * AcsReader — abstract base class for ACS (Advanced Card Systems) readers.
 *
 * Provides shared pseudo-APDU commands available on both the ACR122U
 * and the ACR1252U (via its ACR122U compatibility layer): firmware
 * version retrieval, PICC operating parameter get/set, and combined
 * LED + buzzer control.
 *
 * Subclasses add model-specific commands (e.g. {@link ACR122UReader}
 * adds the standalone buzzer-on-detection toggle).
 */
...
/**
 * Abstract base class for ACS PC/SC readers (ACR122U, ACR1252U, etc.).
 *
 * These readers share a set of pseudo-APDU commands for controlling
 * LEDs, buzzers, and PICC parameters. On the ACR1252U, these commands
 * are available through the ACR122U compatibility layer.
 *
 ...
 */
export abstract class AcsReader extends Reader {
  ...
  /**
   * Controls the reader's LEDs and buzzer.
   *
   * ACS readers have two LEDs (red and green) and a piezo buzzer.
   ...
   * reader always reports the current LED state in its response.
   *
   * **Requires a card:** This command is a pseudo-APDU sent via
   * `SCardTransmit` and will only succeed while a card is present
   * on the reader.
